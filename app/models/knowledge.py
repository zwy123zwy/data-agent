"""
Knowledge ORM 模型 — 对齐 Java agent_knowledge 表
"""
from sqlalchemy import Column, Integer, String, Text, DateTime, ForeignKey, JSON, BigInteger, Index
...
class Knowledge(Base):
    ...
    question = Column(Text, comment="FAQ/QA 的问题（对齐Java）")
    is_recall = Column(Integer, default=1, comment="业务状态: 1=召回, 0=非召回（对齐Java）")
    embedding_id = Column(String(100), comment="向量ID（Chroma, Python 扩展）")
    embedding_status = Column(String(20), default="PENDING", comment="向量化状态: PENDING/PROCESSING/COMPLETED/FAILED（对齐Java）")
    error_msg = Column(String(255), comment="操作失败的错误信息（对齐Java）")
    metadata_ = Column("metadata", JSON, comment="元数据（Python 扩展）")
    enabled = Column(Integer, default=1, nullable=False, comment="是否启用: 0/1（Python 扩展）")

    # 文件管理字段（对齐Java）
    source_filename = Column(String(500), comment="源文件名（对齐Java）")
    file_path = Column(String(500), comment="文件路径（对齐Java）")
    file_size = Column(BigInteger, comment="文件大小（字节，对齐Java）")
    file_type = Column(String(255), comment="文件类型（对齐Java）")
    ...
    # 索引 — 对齐 Java
    __table_args__ = (
        Index("idx_agent_id_status", "agent_id", "is_recall"),
