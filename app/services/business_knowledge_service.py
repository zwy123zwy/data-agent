...
from ..schemas.business_knowledge import BusinessKnowledgeCreate, BusinessKnowledgeUpdate, BusinessKnowledgeResponse
...
class BusinessKnowledgeService:
    ...
    @staticmethod
    async def create(db: AsyncSession, dto: BusinessKnowledgeCreate) -> BusinessKnowledgeResponse:
    ...
    @staticmethod
    async def update(db: AsyncSession, id: int, dto: BusinessKnowledgeUpdate) -> Optional[BusinessKnowledgeResponse]:
        result = await db.execute(
            select(BusinessKnowledge).where(
                and_(BusinessKnowledge.id == id, BusinessKnowledge.is_deleted == 0)
            )
    ...
    @staticmethod
    async def set_recall(db: AsyncSession, id: int, is_recall: bool) -> bool:
        result = await db.execute(
            select(BusinessKnowledge).where(
                and_(BusinessKnowledge.id == id, BusinessKnowledge.is_deleted == 0)
            )
        ...
        if not bk:
            return False
        bk.is_recall = 1 if is_recall else 0
        await db.flush()
        return True
