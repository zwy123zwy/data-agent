...
async def create_semantic_model(
    agent_id: int, model_data: SemanticModelCreate, db: AsyncSession = Depends(get_db),
):
    """创建语义模型"""
...
@router.get("", response_model=dict, summary="列出语义模型")
...
@router.get("/{model_id}", response_model=SemanticModelResponse, summary="获取语义模型详情")
async def get_semantic_model(agent_id: int, model_id: int, db: AsyncSession = Depends(get_db)):
    """获取语义模型详情"""
...
@router.put("/{model_id}", response_model=SemanticModelResponse, summary="更新语义模型")
async def update_semantic_model(
    agent_id: int, model_id: int, model_data: SemanticModelUpdate, db: AsyncSession = Depends(get_db),
):
    """更新语义模型"""
...
@router.delete("/{model_id}", status_code=204, summary="删除语义模型")
async def delete_semantic_model(agent_id: int, model_id: int, db: AsyncSession = Depends(get_db)):
    """删除语义模型"""
    existing = await SemanticModelService.get_semantic_model(db, model_id)
    if not existing or existing.agent_id != agent_id:
        raise HTTPException(status_code=404, detail="语义模型不存在")
...
@router.post("/search", response_model=List[SemanticModelResponse], summary="搜索语义模型")
async def search_semantic_models(
    agent_id: int, search_request: SemanticModelSearchRequest, db: AsyncSession = Depends(get_db),
):
    """搜索语义模型"""
    agent = await AgentService.get_agent(db, agent_id)
    if not agent:
