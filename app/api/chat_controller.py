...
@router.get("/agent/{agent_id}/sessions", response_model=list[ChatSessionResponse], summary="获取会话列表")
...
@router.post("/agent/{agent_id}/sessions", response_model=ChatSessionResponse, status_code=201, summary="创建会话")
async def create_session(
    ...
    if not agent:
        raise HTTPException(status_code=404, detail="Agent 不存在")
    title = body.get("title", "新对话") if body else "新对话"
...
@router.get("/sessions/{session_id}/messages", response_model=list[ChatMessageResponse], summary="获取消息列表")
...
@router.post("/sessions/{session_id}/messages", response_model=ChatMessageResponse, status_code=201, summary="保存消息")
...
async def pin_session(
    session_id: str,
    isPinned: bool = Query(..., description="true=置顶, false=取消置顶"),
    db: AsyncSession = Depends(get_db),
):
